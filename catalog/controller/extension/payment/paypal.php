...
class ControllerExtensionPaymentPayPal extends Controller {
	...
	public function createOrder() {					
		...
		if ($this->cart->hasShipping()) {
			...
			$shipping_info['address']['admin_area_1'] = 'DERBY'; //$order_info['shipping_zone'];
			$shipping_info['address']['admin_area_2'] = 'DERBY' ;//$order_info['shipping_city'];
			...
			if ($order_info['shipping_country_id']) {
                ...
                $country_info = $this->model_localisation_country->TSGgetCountryID($order_info['shipping_country_id']);
			
				if ($country_info) {
					$shipping_info['address']['country_code'] = $country_info['iso2'];
				}
			}
