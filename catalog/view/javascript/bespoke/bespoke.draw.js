...
var bespokeDrawer = function(drawObject, options) {
  ...
  this.resetSign = function(){
    ...
    vars.signBackground = '#000000';
    vars.signPanel = {};
    vars.signSymbols = {};

    vars.signSymbolFrame = {};
    vars.signTextFrame = {};
    vars.signTextPanels = {};
  ...
  this.getTexts = function() {
      ...
      textOutStr = JSON.stringify(textOutArr);
  ...
  var rescaleSymbols = function(symbolsArr) {
    ...
    symbolsArr.forEach(function(item, index) {
      ...
      svgid = root.drawSVG.id();
      tmpSymbol = SVG.get('symbol-' + index + '_'+ svgid);

      if (tmpSymbol != null)
        console.log('symbol-' + index + ' ' + tmpSymbol.width() + ' ' + tmpSymbol.height());
...
*/
  ...
  var DrawTextPanelBlocks = function(textPanel) {
    ...
    textLines.forEach(function(textObject, textIndex) {
      ...
      }).id('text-block-' + textPanel.id + '-' + textIndex+'_'+svgid);
  ...
  var reDrawTextBlock = function(panelID, blockID) {
    ...
    var drawTextBlockID = 'text-block-' + textPanel.id + '-' + textBlockIndex+'_'+svgid;
    if (textObject.colour) {
      textLineColor = textObject.colour;
    } else {
      textLineColor = textPanel.baseTextColour;
...
*/
  ...
  var reDrawTextPanel = function(textPanel, textPanelIndex) {
      ...
      textBLockLines.forEach(function(textLines, blockID){
        //textBlockBound = getTextBlockBoundsByID(panelID, blockID);
        let trueBlockID = textLines.id
        textBlockBound = getTextBlockBoundsByID(panelID, trueBlockID);
        //record the bounds for this text block for later use
        textLines.boundBox = textBlockBound;
        ...
        verticalAlignTextInBlock(panelID, trueBlockID); //, textBlockBound, newTextInfo['linesHeight']);
        horizontalAlignTextInBlock(panelID, trueBlockID, textLines.anchor);
        reDrawTextBlock(panelID, trueBlockID);
      });
